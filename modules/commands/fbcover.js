module.exports.config = {
  ...
  module.exports.run = async function({ api, args, event }) {
    ...
    if(args[0] == "list") {
        ...
        const res = await axios.get("https://docs-api.nguyenhaidang.ml/taoanhdep/list");
     ...
     var char2 = args[2];
      if(!args[2])
      {
        const res = await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=name&name=${char}`);
        var id = res.data.ID;
        return api.sendMessage(`ID của ${char} là: ${id}`, threadID, messageID);
      } 
      else
      {
        const res = await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=name&name=${char} ${char2}`);
        var id = res.data.ID;
        return api.sendMessage(`ID của ${char} là: ${id}`, threadID, messageID);
      }
  ...
  module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    switch (handleReply.type) {
      case "characters": {
        ...
        const res = await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=id&id=${id}`);
      ...
      case "create": {
        ...
        return api.sendMessage(`Đang tạo ảnh ...`, event.threadID, async (error, info) => {
          ...
          var imag = (await axios.get(`https://docs-api.nguyenhaidang.ml/fbcover/v2?name=${encodeURIComponent(name_)}&id=${idchar}&subname=${encodeURIComponent(subname_)}&color=${encodeURIComponent(color_)}`, {
            responseType: "stream"
          })).data;
          var msg = {
